...
def run_simulation(args):
    seed, peddamp, pedBodyF, beamFreq, beamdamp, density,Length, Width = args
    if seed is not None:
        np.random.seed(seed)
    ...
    # ---------------- Frequency grid ----------------
    ExcitationFrequency = np.arange(0.25, 6.0, 0.05)
    ...
    for i, ti in enumerate(t):
        F_modal = modal_force_from_udl_2harm(
            ti,                 # time
            width,              # deck width
            beamFreq,               # excitation / walking frequency
            density,            # pedestrian density
            modalDampingRatio,  # xi
            numped,             # number of pedestrians
            length * width,     # deck area S
            length,             # bridge length
            modalmass,          # list of modal masses
            func_list           # mode shapes
        )
    
        # first modal force = first component
        Q1_t[i] = F_modal[0, 0]
    ...
    # process eq_UDL
    eq_mid, eq_amp, eq_centered, eq_abs_centered, eq_norm, eq_abs_norm = center_abs_normalize(eq_UDL)

    # process realUDL
    real_mid, real_amp, real_centered, real_abs_centered, real_norm, real_abs_norm = center_abs_normalize(realUDL)
    ...
    return t, modalpedmass_i, eq_UDL, realUDL'''